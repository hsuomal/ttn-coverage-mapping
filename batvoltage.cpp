...
float getBatteryVoltage()
{
  ...
  //Characterize ADC at particular atten (factory calibration from fuses)
  esp_adc_cal_characteristics_t adc_chars;
  esp_adc_cal_value_t val_type = esp_adc_cal_characterize(ADC_UNIT_2, ADC_ATTEN_6db, ADC_WIDTH_BIT_12, 1100, &adc_chars);

  // average some samples to avoid any low voltage spikes
  uint8_t samples=4;
  voltage=0;
  ...
  // Shutdown at 3.1V (input impedance lowers reading, this is close to 3.5V in reality on test device)
  if(voltage < 3.1) {
